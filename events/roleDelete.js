...
const BURST_THRESHOLD = 2; // 2 suppressions en rafale
...
module.exports = function (client) {
  client.on('roleDelete', async (role) => {
    ...
    try {
      ...
      if (!config || !config.roleDelete) return;
      ...
      if (!botMember || !botMember.permissions.has(PermissionsBitField.Flags.ViewAuditLog)) {
        ...
        try {
          await safeSend(`⚠️ **Anti-Raid** : Permission "View Audit Log" manquante. Impossible de surveiller les suppressions de rôles.`);
        } catch {
      ...
      // 🧤 Protection contre les guildes supprimées ou inconnues
      let executor;
      try {
        ...
        if (err.code === 10004) {
          console.warn(`[AntiRaid] 🚫 Impossible de récupérer les logs : guilde inconnue (${guild.id}).`);
          return;
        }
        if (err.code === 50013) {
      ...
      const now = Date.now();
      const threshold = config.thresholds?.maxRoleDeletesPer800ms || 3;
      ...
      if (recent.length >= BURST_THRESHOLD) {
        ...
        if (burstRecent.length >= BURST_THRESHOLD) {
          console.warn(`[AntiRaid] 🚨 RAFALE DÉTECTÉE : ${executor.tag} a supprimé ${burstRecent.length} rôles en ${BURST_WINDOW}ms !`);
          
          // Procéder au ban immédiat
          userDeletions.set(key, [now]);

          try {
            await safeSend(`🚨 **ALERTE RAFALE** : ${executor.tag} a supprimé ${burstRecent.length} rôles en moins de ${BURST_WINDOW}ms !`);
          } catch (error) {
            console.error("[AntiRaid] Impossible d'envoyer un message de log:", error);
          }

          return await handleBan(executor, guild, botMember, key, safeSend, client, 'Rafale de suppressions de rôles détectée');
        }
      }
      ...
      if (recent.length < threshold) {
        ...
        try {
          await safeSend(`⚠️ ${executor.tag} a supprimé un rôle. (${recent.length}/${threshold}) - Fenêtre: ${DETECTION_WINDOW}ms`);
        } catch (error) {
          console.error("[AntiRaid] Impossible d'envoyer un message de log:", error);
        }
        ...
        return await handleBan(executor, guild, botMember, key, safeSend, client, 'Suppression massive de rôles détectée');
      }
    } catch (err) {
      console.error('[AntiRaid] Erreur inattendue dans roleDelete :', err);
    }
  });
};
